...
async function fetchRanking(genre: string): Promise<Item[]> {
  ...
  const res = await fetch(`${base}/api/(public)/ranking/yearly/${encodeURIComponent(genre)}`, {
    next: { revalidate: 600, tags: [`ranking:yearly:${genre}`] },
  });
  if (!res.ok) return [];
  return res.json();
}
...
export async function generateMetadata({ params }: { params: { genre: string } }) {
  const title = `연간 랭킹 — ${decodeURIComponent(params.genre)} | Genie Webtoon`;
  const description = `연간 ${decodeURIComponent(params.genre)} 장르 인기 웹툰 순위`;
  return { title, description, openGraph: { title, description } };
}

export default async function YearlyRankingPage({ params }: { params: { genre: string } }) {
  const items = await fetchRanking(params.genre);
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
      <h1 className="text-xl sm:text-2xl font-bold">
        연간 랭킹 — {decodeURIComponent(params.genre)}
      </h1>
